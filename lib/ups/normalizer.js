...
this.normalize = function normalize (parsedData, callback) {
    ...
    for (var i = 0; i < parsedData.steps.length; i++)
    {
        ...
        var gatheredData = {
            "location": {
                            "address": Model.toTitleCase(step.AddressLine),
                            "city": Model.toTitleCase(step.City),
                            "zip": step.location.PostalCode,
                            "state": step.location.StateProvinceCode,
                            "country": step.location.CountryCode
                        },
